...
#+BEGIN_SRC emacs-lisp
  ...
  (dim-minor-name 'projectile-mode " Proj")
  (setq pe/width 20)
...
Weave with ~F7~; export with ~C-F7~; use =knitr::pandoc= with options
in =.pandoc/config.pandoc= with ~F8~, and render a bookdown with
~C-F8~:
