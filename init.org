...
#+BEGIN_SRC emacs-lisp
  (setq package-list '(
                       ...
                       dimmer
                       ;; dired+    ; EmacsWiki not on MELPA anymore
                       dired-narrow
                       dired-quick-sort
                       dired-sidebar
                       dired-subtree
...
20%). Coordinates are for South Florida:
...
#+BEGIN_SRC emacs-lisp
(setq
 ;; Maximum size of 30% of screen instead of 25%
...
(check 'customize' branch for all Ivy functions: https://github.com/Yevgnen/ivy-rich/tree/customize)

#+BEGIN_SRC emacs-lisp
  (ivy-rich-mode 1)  
  (setq ivy-virtual-abbreviate 'full                   ; Align virtual buffers
        ivy-rich-switch-buffer-align-virtual-buffer t
        ivy-rich-path-style 'abbrev)          ; Abbreviate file name with ~
#+END_SRC
...
# [[https://github.com/bbatsov/super-save/][Super-save]] to automatically save files on special events (e.g. switch
# between buffers, an Emacs frame loses focus, etc.):
...
(=magit-gitflow=, started with ~C-f~) and Forge
...
additional PDFs (starting with BibTeX key) are also detected;
fields to be displayed: PDF, author, title, year, and
...
# (check ESS news 18-10: "The ESS initialization process has been
# further streamlined. If you update the autoloads (which installation
# from package-install does), you should not need to (require
# 'ess-site) at all, as autoloads should automatically load ESS when
# it is needed (e.g. the first time an R buffer is opened). In order
# to defer loading your ESS config, you may want to do something like
# (with-require-after-load "ess" <ess-config-here>) in your Emacs init
# file. Users of the popular use-package Emacs package can now do
...
#+BEGIN_SRC emacs-lisp
  (add-hook 'ess-mode-hook 
            (lambda ()
              (local-set-key (kbd "C-=") 'ess-cycle-assign) ; C-= cycles through <- = <<- etc.
              (flyspell-prog-mode)
              (prettify-symbols-mode)   ; Change <- into ←, etc.
              ...
              (define-key ess-r-mode-map ";" #'ess-insert-assign) ; `;` inserts ` <- ` (type twice for a real ";")
