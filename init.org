...
comments at 20%). **Notes:** There is currently [[https://github.com/LionyxML/auto-dark-emacs/issues/90][a bug]] with GNOME that requires
inverting dark and light themes. Another bug in Emacs (maybe) makes incomplete
color theme change in unselected windows: only the background color changes, and
not font faces, until the window is visited.

#+BEGIN_SRC emacs-lisp
(setq nord-comment-brightness 20)
(setq custom-safe-themes t)
;; (setq auto-dark-themes '((tango-plus) (nord)))
(setq auto-dark-themes '((nord) (tango-plus)))
(auto-dark-mode)
#+END_SRC
...
=dimmer=). Note: There is a unfixed bug in =dimmer= that requires modifying the
definitions of variables in the function =dimmer-face-color= in =dimmer.el=:
...
By default, lines soft wrapped at word boundary (visual line mode), with fringe
indicators in the fringe column, except for Dired, Grep, and Imenu-list buffers;
~F10~ toggles between word wrapping (i.e. visual line mode), line truncation and
regular wrapping:
...
(defun my-toggle-line-display ()
  "Cycle between truncate lines, regular wrapping, and visual-line-mode."
  (interactive)
  (cond
   ;; Case 1: Currently visual-line-mode -> switch to truncate
   (visual-line-mode
    (visual-line-mode -1)
    (setq truncate-lines t)
    (message "Line display: Truncate lines"))
   ;; Case 2: Currently truncating -> switch to regular wrapping
   (truncate-lines
    (setq truncate-lines nil)
    (message "Line display: Regular wrapping"))
   ;; Case 3: Regular wrapping -> switch to visual-line-mode
   (t
    (visual-line-mode 1)
    (message "Line display: Word wrapping (visual-line-mode)"))))

(global-set-key (kbd "<f10>") #'my-toggle-line-display)
#+END_SRC
...
Highlight columns ([[https://www.emacswiki.org/emacs/col-highlight.el][col-highlight]]): ~C-S-F10~ to toggle column highlight mode
(NB: col-highlight loaded as a function as EmacsWiki is no longer on MELPA).
...
[[https://github.com/DamienCassou/beginend][Expand region]] with ~C-@~ (then continue to expand by pressing ~@~ or
...
Ivy-resume (go back to state of last search) with ~C-S-c~:

#+BEGIN_SRC emacs-lisp
(global-set-key (kbd "C-S-c") 'ivy-resume)
#+END_SRC
...
Use Counsel (=counsel-rg=) for recursive Swiper-like search with ~C-S-s~:
...
Auto-completion in the buffer with Complete Anything (=company=), with no delay
and aggressive completion. Note that ~TAB~ complete the current word if there is
only one suggestion or complete the common part and cycle through suggestions if
there are several; ~RET~ is only active after interaction with the selection;
~SPC~ is disabled with respect to completion:
...
(defun my/company-smart-tab ()
  "If company popup is active, expand common prefix or complete selection.
Otherwise, indent the current line or region."
  (interactive)
  (if (company--active-p)
      ;; Company popup is active
      (let ((before (buffer-string)))
        (company-complete-common)
        (when (string= (buffer-string) before)
          (company-complete-selection)))
    ;; No popup → fallback to normal indentation
...
(with-eval-after-load 'company
  ;; Remove all default TAB variants
  (dolist (key '("TAB" "<tab>" "C-i"))
    (define-key company-active-map (kbd key) nil))

  ;; Bind smart TAB
  (define-key company-active-map (kbd "TAB") #'my/company-smart-tab)
  ...
  ;; Optional: prevent SPACE from triggering completion
  (define-key company-active-map (kbd "SPC") nil))
...
=revert-buffer= requires confirmation. Here is the same function without
confirmation, bound to ~C-S-r~:
...
Emacs built-in spell check package is Ispell. A good approach is to use
=Hunspell= as the spell check engine (needs to be installed), with "en_US" as
the default dictionary (~C-S-F12~ to change dictionary). =Flyspell= (spell check
on the fly) is enabled by default in all text files (~C-F12~ to toggle
Flyspell), and in programming mode (only in the comments) in programming files,
and font face is customized so that it only underlines typos without adding
background color. ~F12~ (or middle click) opens the list of correction
suggestions:
...
(custom-set-faces
 '(flyspell-incorrect ((t (:underline (:color "red" :style wave) :background nil))))
 '(flyspell-duplicate ((t (:underline (:color "orange" :style wave) :background nil)))))
...
Turn on indent mode, and use nice UTF-8 bullet points, and display emphasis
over multiple lines:
...
to insert =**TODO**= and cycle between **TODO**/**CANCEL**/**DONE**/nothing,
~C-S-F5~ to display an interactive list of to-do items in a Grep top buffer):
...
(add-hook 'markdown-mode-hook 
          (lambda ()
            ...
            (local-set-key [C-S-f5] 'md-todo-dashboard)
            ))
#+END_SRC
...
(setq initial-buffer-choice
      (lambda ()
        (if (buffer-file-name)          ; If a file is selected, just open it
            (current-buffer)
          (start-here))))               ; Else run start-here
#+END_SRC
