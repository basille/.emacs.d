...
Lines soft wrapped at word boundary (with fringe indicators in the fringe column):
...
Opens the =Start Here.md= file if no file is selected, with NeoTree on the left
(a little bit buggy):
