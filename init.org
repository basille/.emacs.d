...
Use the theme [[https://github.com/purcell/color-theme-sanityinc-tomorrow][Tomorrow-eighties]] for Emacs:

#+BEGIN_SRC emacs-lisp
  ;; (load-theme 'sanityinc-tomorrow-eighties t)
#+END_SRC

With the help of the package [[https://github.com/hadronzoo/theme-changer][Theme Changer]] (not in ELPA), Emacs can
switch themes based on sunrise and sunset. I will use
[[https://github.com/waymondo/apropospriate-theme][apropospriate-light-theme]] during the day, and [[https://github.com/arcticicestudio/nord-emacs][Nord Emacs]] at night
(coordinates are for South Florida):

#+BEGIN_SRC emacs-lisp
  (setq calendar-location-name "Davie, FL")
...
code (~C-S-return~); weave with ~F7~; use =knitr::pandoc= with options
in =.pandoc/config.pandoc= with ~C-F7~:

#+BEGIN_SRC emacs-lisp
  (add-hook 'markdown-mode-hook 
            (lambda ()
              (load-library "rmd-library")
  ...
  (with-eval-after-load 'polymode
    ...
    (define-key polymode-mode-map [(ctrl f7)] #'rmd-pandoc-custom))
#+END_SRC
