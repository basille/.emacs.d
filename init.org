...
#+BEGIN_SRC emacs-lisp
  ...
  (setq pe/width 20)
  (setq pe/cache-directory
        (concat user-emacs-directory "cache/project-explorer-cache/"))
...
(notably forward and inverse search, RefTeX, compilation as PDF, Math
mode):
...
Weave with ~F8~; export (pandoc) with ~C-F8~; use =knitr::pandoc= with
options in =.pandoc/config.pandoc= with ~C-S-F8~:

#+BEGIN_SRC emacs-lisp
  (add-hook 'markdown-mode-hook 
            (lambda ()
              (load-library "md-insert-r-code.el")
              (local-set-key [C-return] 'md-fenced-r-code-block)
              (local-set-key [C-S-return] 'md-inline-r-code)
              (load-library "md-ess-pandoc-custom.el")
              ;; (local-set-key [(ctrl shift f8)] 'md-ess-pandoc-custom)
              ))
  (with-eval-after-load 'polymode
    (define-key polymode-mode-map [(f8)] #'polymode-weave)
    (define-key polymode-mode-map [(ctrl f8)] #'polymode-export)
    (define-key polymode-mode-map [(ctrl shift f8)] #'md-ess-pandoc-custom))
#+END_SRC
...
SQL works already well out of the box. However, more configuration is
required to have a beautiful code and readable output. First of all,
[[https://github.com/bsvingen/sql-indent][sql-indent]] allows to indent correctly SQL code:
...
In the output, we first make sure that lines are not truncated:
