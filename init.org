...
directories first, intelligently guess where to copy (other window),
and does not ask for confirmation for recursive copies and
deletes. Switch to WDired mode (to 'write' file names) with ~C-S-F5~,
and go to bookmarks with ~$~:
...
TRAMP history of connections in =.emacs.d/cache/tramp=:

#+BEGIN_SRC emacs-lisp
  (setq tramp-persistency-file-name (concat user-emacs-directory "cache/tramp"))
#+END_SRC
